...
#if (cfgSysShowErrorOnUart == cfgTrue)
const Tchar OsErrStr[] PROGMEM = "OS Err: ";
#endif

void portShowError(Tuint08 uiMessage, Tuint08 uiCallId, Tuint08 uiInfoTask)
{
#if (cfgSysShowErrorOnUart == cfgTrue)
	libuart_puts((Tchar*)OsErrStr);
	libuart_sendraw(uiMessage);
	libuart_sendraw(uiCallId);
	libuart_sendraw(uiInfoTask);
	libuart_sendraw('\n'); }
#else
